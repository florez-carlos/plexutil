...
class PlexUtilLibraryConfigService(Static):
    ...
    def get_plexutil_library_config(
            ...
            library_name: str) -> PlexUtilConfigEntity:

        return PlexUtilLibraryConfigEntity.select().where(
            (PlexUtilLibraryConfigEntity.name == library_type)
            & (PlexUtilLibraryConfigEntity.library_type == library_name)
        ).get()

    @staticmethod
    def add_plexutil_library_config(plexutil_config_entity: PlexUtilConfigEntity) -> None:

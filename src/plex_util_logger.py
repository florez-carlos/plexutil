...
class PlexUtilLogger(metaclass=SingletonMeta):
    def __init__(self, log_dir: Path) -> None:
        if not hasattr(self, 'initialized'):  # Avoid reinitialization
            ...
            logging_config_path = PathOps.get_project_root() / "src" / "config" / "log_config.yaml"
            # Load YAML config
            logging_config = FileImporter.get_logging_config(logging_config_path)
            # Rewrite contents of YAML config to accomodate for date based log file names
            logging_config['handlers']['regular_file_handler']['filename'] = log_dir / log_file_name
